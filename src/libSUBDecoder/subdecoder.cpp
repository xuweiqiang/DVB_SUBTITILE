...
public:
	void NotifyRenderCB(unsigned long PTS, unsigned long Duration, int Regions, int Number);
	LPVOID GetDvbSub();
			...
			if (region_num)
			{
				...
				if (pDvbSUB->cSubSync.Synchronize(elem.pts, 10000))  //unit: ms
				{
					...
					for (int i=0;i<region_num;i++)
					{
						...
						pThis->NotifyRenderCB(elem.pts, pDvbSUB->sDisplayInfo.duration_time, region_num, i+1);
...
static DWORD WINAPI SubVanishThread(LPVOID pArg)
{
	...
	while (pThis->IsVanishThreadGoing())
	{
        ...
        for (int i=0; i<10; i++) {
            Sleep(QUERY_DURATION/10);
            if (pThis->IsVanishThreadGoing() == false) {
                break;
            }
        }
	}
...
void CSUBDecoder::NotifyRenderCB(unsigned long PTS, unsigned long Duration, int Regions, int Number)
{
	CI_SUB_Draw_Info Info;
